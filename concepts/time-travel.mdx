...
description: "Branch, rewind, and restore agent conversation states without re-executing past LLM turns."
---

Every turn processed by `kenpachi` is automatically snapshotted in memory. Time-Travel Context allows you to rewind an agent's history back to any previous state and branch off in a new direction—without re-querying the model or paying extra token costs for turns you have already run.

<Tip>
**Think of it like a save state in a video game.** You can save before entering a path, test an outcome, and jump back to the exact checkpoint if you want to try a different approach.
</Tip>
